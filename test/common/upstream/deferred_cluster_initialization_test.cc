...
// Verifies that registered update callbacks are invoked when deferred clusters are added
// unbatched, and invoking the command closure inflates the cluster inline.
TEST_P(StaticClusterTest, UnbatchedDeferredClusterCallbackInflation) {
  auto bootstrap = parseBootstrapFromV3YamlEnableDeferredCluster("static_resources:\n");
  create(bootstrap);
  EXPECT_EQ(readGauge("thread_local_cluster_manager.test_thread.clusters_inflated"), 0);
  ...
  EXPECT_CALL(cb1, onClusterAddOrUpdate(_, _))
      .WillOnce(Invoke([&callback_order](absl::string_view name, ThreadLocalClusterCommand& cmd) {
  ...
  auto cluster = defaultStaticCluster("unbatched_deferred");
  setClusterType(cluster, getStaticClusterType());

  // Add deferred cluster unbatched (without an active batch context)
  EXPECT_TRUE(*cluster_manager_->addOrUpdateCluster(cluster, "v1"));

  EXPECT_TRUE(cluster_manager_->hasCluster("unbatched_deferred"));
  // Invoking the command closure inside callbacks must have inflated the cluster inline.
  EXPECT_EQ(readGauge("thread_local_cluster_manager.test_thread.clusters_inflated"), 1);
  EXPECT_NE(nullptr, cluster_manager_->getThreadLocalCluster("unbatched_deferred"));

  const std::vector<std::string> expected_order = {"cb2:unbatched_deferred",
                                                   "cb1:unbatched_deferred"};
